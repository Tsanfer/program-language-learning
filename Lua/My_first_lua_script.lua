...
-- 文件 I/O
local function WriteTableToFile(file, table)
    local file = io.open(file, 'w+') -- 形参可与函数内局部变量同名
    if file then -- 如果打开（创建）文件成功
        io.output(file)
        for key, line in ipairs(table) do
            print(key .. ':' .. line) -- 打印行数与内容
            io.write(line .. '\n') -- 写入文件
        end
    end
    io.close(file)
